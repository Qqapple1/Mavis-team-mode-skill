...
> Numbers are heuristic estimates from `scripts/benchmark_tokens.py` on v1.4.0. Re-run the script for current values; they drift as files change.
...
```yaml
description: "Recreates the TeamForge workflow (Leader + Workers + Verifier) inside Zcode 3.4.2+. Use for: 'team mode', 'split into subtasks', 'verify the result', '用 teamforge'."
```
